...
export default function ListaEquiposCliente() {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      setEquipos(lista.filter(e => (e.cliente || "Sin cliente") === cliente));
    };
    cargar();
  }, [cliente]);
    ...
    <div style={s.page}>
      ...
      <div style={s.navbar}>
        <div style={s.navLeft}>
          ...
          <div style={s.divider}></div>
          <span style={s.navCliente}>{cliente}</span>
        </div>
        <div style={s.navBtns}>
          <button style={s.btnExcel} onClick={() => exportarExcel(cliente, equipos)}>Excel</button>
          <button style={s.btnPdf} onClick={() => exportarPDF(cliente, equipos)}>PDF</button>
        </div>
      </div>
...
const s = {
  ...
  navCliente: { fontSize: "14px", fontWeight: 500, color: "#222" },
