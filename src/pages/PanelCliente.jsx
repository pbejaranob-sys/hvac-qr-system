...
// ---- Hook responsivo: detecta si es dispositivo móvil real (no por ancho de ventana) ----
const useIsMobile = () => {
  ...
  React.useEffect(() => {
    // Re-evaluar solo si cambia entre orientaciones en dispositivo real
...
export default function PanelCliente() {
  ...
  // ---- DISEÑO MÓVIL (< 768px) ----
  if (isMobile) return <PanelClienteMobile
    equipos={equipos} sedes={sedes} usuario={usuario}
    averias={averias} vistaActual={vistaActual} sedeActual={sedeActual}
    setVistaActual={setVistaActual} setSedeActual={setSedeActual}
    filtroEstado={filtroEstado} setFiltroEstado={setFiltroEstado}
    filtroTipoEquipo={filtroTipoEquipo} setFiltroTipoEquipo={setFiltroTipoEquipo}
    filtroMes={filtroMes} setFiltroMes={setFiltroMes}
    pisosSeleccionados={pisosSeleccionados} setPisosSeleccionados={setPisosSeleccionados}
    obsAbierto={obsAbierto} setObsAbierto={setObsAbierto}
    equiposFiltrados={equiposFiltrados} equiposMostrados={equiposMostrados}
    averiasSedeActual={averiasSedeActual}
    tot={tot} op={op} obs={obs} fs={fs}
    handleLogout={handleLogout}
    abrirEmergencias={abrirEmergencias} abrirEmergenciasSede={abrirEmergenciasSede}
    abrirHistorial={abrirHistorial} abrirDetalleAveria={abrirDetalleAveria}
    detalleAveria={detalleAveria} cerrarDetalleAveria={cerrarDetalleAveria}
    marcarAveriaAtendida={marcarAveriaAtendida}
    listaEmergenciaSede={listaEmergenciaSede} setListaEmergenciaSede={setListaEmergenciaSede}
    exportarExcel={exportarExcel} exportarPDF={exportarPDF}
    navigate={navigate} getBadge={getBadge} fechaAMesAnio={fechaAMesAnio}
    mesesDisponibles={mesesDisponibles} pisos={pisos} tiposEquipo={tiposEquipo}
    getObs={getObs} getRec={getRec}
    historialAbierto={historialAbierto} historialAverias={historialAverias}
    historialSedeFiltro={historialSedeFiltro} cargandoHistorial={cargandoHistorial}
  />;
...
// ============================================================
// COMPONENTE MÓVIL — solo para pantallas < 768px
// ============================================================
function PanelClienteMobile({
  equipos, sedes, usuario, averias,
  vistaActual, sedeActual, setVistaActual, setSedeActual,
  filtroEstado, setFiltroEstado, filtroTipoEquipo, setFiltroTipoEquipo,
  filtroMes, setFiltroMes, pisosSeleccionados, setPisosSeleccionados,
  obsAbierto, setObsAbierto,
  equiposFiltrados, equiposMostrados, averiasSedeActual,
  tot, op, obs, fs,
  handleLogout, abrirEmergencias, abrirEmergenciasSede,
  abrirHistorial, abrirDetalleAveria, detalleAveria,
  cerrarDetalleAveria, marcarAveriaAtendida,
  listaEmergenciaSede, setListaEmergenciaSede,
  exportarExcel, exportarPDF,
  navigate, getBadge, fechaAMesAnio,
  mesesDisponibles, pisos, tiposEquipo,
  getObs, getRec,
  historialAbierto, historialAverias, historialSedeFiltro, cargandoHistorial,
}) {
  const [equipoInfo, setEquipoInfo] = React.useState(null);
  const FONT = "'Manrope', -apple-system, sans-serif";
  ...
  if (equipoInfo) {
    ...
    const datosGrid = [
      ...
      ["Refrigerante", equipoInfo.tipoRefrigerante || "—"],
      ["Voltaje", equipoInfo.voltaje ? `${equipoInfo.voltaje}V` : "—"],
      ["Amperaje", equipoInfo.amperaje ? `${equipoInfo.amperaje}A` : "—"],
      ["Fases", equipoInfo.fases || "—"], ["Tipo", equipoInfo.tipoEquipo || "—"],
    ];
    return (
      <div style={{ ...m.page }}>
        <div style={m.navbar}>
          <button style={m.btnNavBack} onClick={() => setEquipoInfo(null)}>←</button>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={m.navTitleSm}>Info del equipo</div>
            <div style={m.navSubtitle}>{sedeActual?.nombre || usuario?.empresa}</div>
          </div>
        </div>
        <div style={m.listaPad}>
          <div style={m.infoCard}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={m.infoIconBox}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 800, fontSize: 15, color: "#12245e" }}>{equipoInfo.marca} {equipoInfo.modelo}</div>
                {getBadge(equipoInfo.estado)}
              </div>
            </div>
            <div style={{ borderTop: "1px solid #eef1f6", paddingTop: 10, fontSize: 13, color: "#26314d", fontWeight: 600 }}>
              Piso {equipoInfo.piso} · {equipoInfo.ambiente}
            </div>
          </div>
          <div style={m.infoCard}>
            <div style={m.infoSecTitulo}>DATOS DEL EQUIPO</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {datosGrid.map(([label, value]) => (
                <div key={label}>
                  <div style={{ fontWeight: 600, fontSize: 10.5, color: "#8a92a6", textTransform: "uppercase" }}>{label}</div>
                  <div style={{ fontWeight: 700, fontSize: 13.5, color: "#12245e", marginTop: 3 }}>{value}</div>
                </div>
              ))}
            </div>
          </div>
          {equipoInfo.ultimoMantenimiento && (
            <div style={m.infoCard}>
          ...
          {obsArr.length > 0 && (
            <div style={m.infoCard}>
              <div style={m.infoSecTitulo}>OBSERVACIONES</div>
              {obsArr.map((o, idx) => (
                <div key={idx} style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                  <div style={{ flex: 2, background: "#fff8e6", borderRadius: 8, padding: "8px 10px" }}>
                    <div style={{ fontWeight: 700, fontSize: 10, color: "#8a92a6", marginBottom: 3 }}>OBSERVACIÓN</div>
                    <div style={{ fontWeight: 600, fontSize: 12, color: "#8a5b0a" }}>{o.texto}</div>
                  </div>
                  <div style={{ flex: 2, background: "#e6f7ec", borderRadius: 8, padding: "8px 10px" }}>
                    <div style={{ fontWeight: 700, fontSize: 10, color: "#8a92a6", marginBottom: 3 }}>RECOMENDACIÓN</div>
                    <div style={{ fontWeight: 600, fontSize: 12, color: "#1c7a44" }}>{recArr[idx] ? (typeof recArr[idx] === "string" ? recArr[idx] : recArr[idx]?.texto || "—") : "—"}</div>
                  </div>
                </div>
              ))}
            </div>
          )}
          <button style={m.btnVolver} onClick={() => setEquipoInfo(null)}>← Volver a la lista</button>
        </div>
      </div>
    );
  }
  ...
  if (vistaActual === "sedes") return (
    <div style={m.page}>
      <div style={m.navbar}>
        <div style={m.navLogoBox}><img src="/assets/hvac-isotipo-filled.png" alt="HVAC" style={{ width: 22, height: 22, objectFit: "contain", filter: "brightness(0) invert(1)" }} /></div>
        <div style={m.navTitle}>{usuario?.empresa}</div>
        <button style={m.btnSalir} onClick={handleLogout}>Salir</button>
      </div>
      <div style={m.listaPad}>
        {sedes.length === 0 ? (
          <div style={{ textAlign: "center", color: "#aab1c2", fontStyle: "italic", padding: "40px 0" }}>No hay sedes registradas</div>
        ) : sedes.map(sede => {
          const eqSede = equipos.filter(e => e.sede === sede.nombre);
          const opS = eqSede.filter(e => e.estado === "Operativo").length;
          const obsS = eqSede.filter(e => e.estado === "Operativo con observaciones").length;
          ...
          const av = colorAvatar(sede.nombre);
          return (
            <div key={sede.id} style={m.sedeCard}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                <div style={{ ...m.sedeAvatar, background: av.bg, color: av.color }}>{initiales(sede.nombre)}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 800, fontSize: 15, color: "#12245e" }}>{sede.nombre}</div>
                  {sede.direccion && <div style={{ fontWeight: 600, fontSize: 12, color: "#9aa2b3", marginTop: 2 }}>{sede.direccion}</div>}
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
                {[["Operativo", opS, "#1c7a44", "#e6f7ec"], ["Con obs.", obsS, "#a8720b", "#fff3d6"], ["Fuera serv.", fsS, fsS > 0 ? "#a52b2b" : "#9aa2b3", fsS > 0 ? "#fdeeee" : "#f4f6fb"], ["Emergencia", averiasSede.length, averiasSede.length > 0 ? "#a52b2b" : "#9aa2b3", averiasSede.length > 0 ? "#fdeeee" : "#f4f6fb"]].map(([lbl, n, color, bg]) => (
                  <div key={lbl} style={{ background: bg, borderRadius: 12, padding: "10px 4px", textAlign: "center", cursor: lbl === "Emergencia" && n > 0 ? "pointer" : "default" }} onClick={() => lbl === "Emergencia" && n > 0 && abrirEmergenciasSede(sede)}>
                    <div style={{ fontWeight: 800, fontSize: 17, color }}>{n}</div>
                    <div style={{ fontWeight: 700, fontSize: 9.5, color, marginTop: 2 }}>{lbl}</div>
                  </div>
                ))}
              </div>
              <button style={m.btnVerSede} onClick={() => { setSedeActual(sede); setVistaActual("equipos"); setFiltroEstado("Todos"); setPisosSeleccionados([]); }}>
                Ver equipos →
              </button>
            </div>
          );
      ...
      {detalleAveria && <ModalAveriaMobile averia={detalleAveria} equipos={equipos} onCerrar={cerrarDetalleAveria} onAtender={marcarAveriaAtendida} navigate={navigate} />}
    </div>
  );
  ...
  return (
    <div style={m.page}>
      <div style={m.navbar}>
        <button style={m.btnNavBack} onClick={() => { if (sedes.length > 0) { setSedeActual(null); setVistaActual("sedes"); } else handleLogout(); }}>←</button>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={m.navTitleSm}>{sedeActual?.nombre || usuario?.empresa}</div>
          <div style={m.navSubtitle}>{usuario?.empresa}</div>
        </div>
        <button style={m.btnSalir} onClick={handleLogout}>Salir</button>
      </div>
      <div style={{ padding: "10px 14px", display: "flex", gap: 8, background: "#ffffff", borderBottom: "1px solid #e7ebf3" }}>
        <button style={m.btnExcel} onClick={() => exportarExcel(usuario?.empresa, equiposFiltrados)}>Excel</button>
        <button style={m.btnPdf} onClick={() => exportarPDF(usuario?.empresa, sedeActual?.nombre, equiposFiltrados)}>PDF</button>
      </div>
      <div style={m.listaPad}>
        {/* Stats */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 4 }}>
          {[["TOTAL", tot, "#1a4fc0", "#e5f0ff", "#c3d6fb", "Todos"], ["OPERATIVOS", op, "#1c7a44", "#e6f7ec", "#c3ecd2", "Operativo"], ["CON OBS.", obs, "#a8720b", "#fff8e6", "#f3dfa3", "Operativo con observaciones"], ["FUERA SERV.", fs, "#a52b2b", "#fdeeee", "#f6d3d3", "Fuera de servicio"]].map(([lbl, n, color, bg, border, filtro]) => (
            <div key={lbl} onClick={() => setFiltroEstado(filtroEstado === filtro ? "Todos" : filtro)}
              style={{ background: bg, border: `1.5px solid ${filtroEstado === filtro ? color : border}`, borderRadius: 14, padding: "16px 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer" }}>
              <div style={{ fontWeight: 800, fontSize: 26, color }}>{n}</div>
              <div style={{ fontWeight: 700, fontSize: 10.5, color: "#6b7488", letterSpacing: "0.05em" }}>{lbl}</div>
            </div>
          ))}
        </div>
        {/* Filtros */}
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <select style={m.filterSel} value={filtroTipoEquipo} onChange={e => setFiltroTipoEquipo(e.target.value)}>
            {tiposEquipo.map(t => <option key={t} value={t}>{t === "Todos" ? "Todos los tipos" : t}</option>)}
          </select>
          <select style={m.filterSel} value={filtroMes} onChange={e => setFiltroMes(e.target.value)}>
            {mesesDisponibles.map(m2 => <option key={m2}>{m2}</option>)}
          </select>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ fontWeight: 800, fontSize: 15, color: "#12245e" }}>Lista de equipos</div>
          <span style={{ background: "#e5f0ff", color: "#1a4fc0", fontWeight: 700, fontSize: 11.5, padding: "3px 9px", borderRadius: 20 }}>{equiposFiltrados.length} equipos</span>
        </div>
        {/* Tarjetas de equipos */}
        {equiposFiltrados.map((equipo, i) => {
          const obsArr = getObs(equipo);
          const abierto = obsAbierto === equipo.id;
          return (
            <div key={equipo.id} style={m.equipoCard}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  <span style={{ fontWeight: 700, color: "#8a92a6", fontSize: 12.5 }}>#{i + 1}</span>
                  {equipo.codigo && <span style={{ background: "#e5f0ff", color: "#1a4fc0", fontWeight: 700, fontSize: 11.5, padding: "3px 8px", borderRadius: 7 }}>{equipo.codigo}</span>}
                  <span style={{ color: "#9aa2b3", fontWeight: 600, fontSize: 12 }}>Piso {equipo.piso || "—"}</span>
                </div>
              ...
              {abierto && (
                <div style={{ background: "#f9fafc", border: "1px solid #eef1f6", borderRadius: 12, padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                  {obsArr.map((o, idx) => {
                    ...
                    return (
                      <div key={idx} style={{ display: "flex", gap: 6 }}>
                        ...
                        <div style={{ flex: 1.5, background: "#fdeeee", borderRadius: 8, padding: "8px 10px" }}>
                          <div style={{ fontWeight: 700, fontSize: 10, color: "#8a92a6", marginBottom: 3 }}>CAUSA</div>
                          <div style={{ fontWeight: 600, fontSize: 12, color: "#a52b2b" }}>{o.causa || "—"}</div>
                        </div>
                        <div style={{ flex: 2, background: "#e6f7ec", borderRadius: 8, padding: "8px 10px" }}>
                          <div style={{ fontWeight: 700, fontSize: 10, color: "#8a92a6", marginBottom: 3 }}>RECOMENDACIÓN</div>
                          <div style={{ fontWeight: 600, fontSize: 12, color: "#1c7a44" }}>{recArr[idx] ? (typeof recArr[idx] === "string" ? recArr[idx] : recArr[idx]?.texto || "—") : "—"}</div>
...
// ---- Modal Avería Móvil ----
function ModalAveriaMobile({ averia, equipos, onCerrar, onAtender, navigate }) {
  const eq = equipos.find(e => e.id === averia.equipoId);
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 999, display: "flex", alignItems: "flex-end", justifyContent: "center", fontFamily: "'Manrope',-apple-system,sans-serif" }} onClick={onCerrar}>
      <div style={{ width: "100%", maxWidth: 480, background: "#ffffff", borderRadius: "20px 20px 0 0", padding: 20 }} onClick={e => e.stopPropagation()}>
        <div style={{ fontWeight: 800, fontSize: 14, color: "#0f1b3d", marginBottom: 12 }}>{eq?.tipoEquipo || "Equipo"} – {averia.ambiente || eq?.ambiente || "-"}</div>
        <div style={{ background: "#fdeeee", border: "1px solid #f6d3d3", borderRadius: 12, padding: 12, marginBottom: 14 }}>
          <div style={{ fontWeight: 600, fontSize: 13, color: "#a52b2b" }}>{averia.mensaje}</div>
        </div>
        {!averia.atendida && (
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            {eq && <button style={{ flex: 1, background: "#1a4fc0", color: "#fff", border: "none", borderRadius: 10, padding: "11px 8px", fontWeight: 700, fontSize: 13, cursor: "pointer" }} onClick={() => navigate(`/protocolo?equipo=${eq.id}`)}>Ver protocolo</button>}
            <button style={{ flex: 1, background: "#1c9a53", color: "#fff", border: "none", borderRadius: 10, padding: "11px 8px", fontWeight: 700, fontSize: 13, cursor: "pointer" }} onClick={() => onAtender(averia.id)}>Marcar atendida</button>
          </div>
        )}
        <button style={{ width: "100%", background: "#f4f6fb", color: "#6b7488", border: "1px solid #e7ebf3", borderRadius: 12, padding: 13, fontWeight: 700, fontSize: 13.5, cursor: "pointer" }} onClick={onCerrar}>Cerrar</button>
      </div>
    </div>
  );
}
...
// ---- Estilos móvil ----
const m = {
  page: { width: "100%", minHeight: "100vh", background: "#eef1f6", fontFamily: "'Manrope',-apple-system,sans-serif", boxSizing: "border-box" },
  navbar: { background: "#ffffff", borderBottom: "1px solid #e7ebf3", padding: "14px 16px", display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 10 },
  navLogoBox: { width: 38, height: 38, minWidth: 38, borderRadius: 10, background: "#1a4fc0", display: "flex", alignItems: "center", justifyContent: "center" },
  navTitle: { flex: 1, fontWeight: 800, fontSize: 14.5, color: "#12245e" },
  navTitleSm: { fontWeight: 800, fontSize: 13.5, color: "#12245e" },
  navSubtitle: { fontWeight: 600, fontSize: 11, color: "#9aa2b3" },
  btnNavBack: { background: "#f4f6fb", border: "1px solid #e7ebf3", borderRadius: 9, width: 36, height: 36, minWidth: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 16 },
  btnSalir: { background: "#f4f6fb", color: "#6b7488", border: "1px solid #e7ebf3", borderRadius: 9, padding: "8px 14px", fontFamily: "'Manrope',-apple-system,sans-serif", fontWeight: 700, fontSize: 12.5, cursor: "pointer" },
  btnExcel: { flex: 1, background: "#e6f7ec", color: "#1c7a44", border: "none", borderRadius: 10, padding: "9px 12px", fontFamily: "'Manrope',-apple-system,sans-serif", fontWeight: 700, fontSize: 12.5, cursor: "pointer" },
  btnPdf: { flex: 1, background: "#fdeeee", color: "#a52b2b", border: "none", borderRadius: 10, padding: "9px 12px", fontFamily: "'Manrope',-apple-system,sans-serif", fontWeight: 700, fontSize: 12.5, cursor: "pointer" },
  listaPad: { padding: "16px 14px", display: "flex", flexDirection: "column", gap: 14 },
  sedeCard: { background: "#ffffff", border: "1px solid #e7ebf3", borderRadius: 18, padding: 18, display: "flex", flexDirection: "column", gap: 14 },
  sedeAvatar: { width: 44, height: 44, minWidth: 44, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 14 },
  btnVerSede: { color: "#1a4fc0", fontWeight: 700, fontSize: 13.5, background: "none", border: "none", textAlign: "left", cursor: "pointer", padding: 0 },
  filterSel: { flex: 1, minWidth: 120, border: "1px solid #dfe6f5", borderRadius: 10, padding: "9px 10px", fontFamily: "'Manrope',-apple-system,sans-serif", fontSize: 12.5, color: "#26314d", background: "#ffffff" },
  equipoCard: { background: "#ffffff", border: "1px solid #e7ebf3", borderRadius: 16, padding: 14, display: "flex", flexDirection: "column", gap: 8 },
  btnInfo: { flex: 1, background: "#1a4fc0", color: "#ffffff", border: "none", borderRadius: 9, padding: "10px 8px", fontFamily: "'Manrope',-apple-system,sans-serif", fontWeight: 700, fontSize: 12.5, cursor: "pointer" },
  btnProtocolo: { flex: 1, background: "#a52b2b", color: "#ffffff", border: "none", borderRadius: 9, padding: "10px 8px", fontFamily: "'Manrope',-apple-system,sans-serif", fontWeight: 700, fontSize: 12.5, cursor: "pointer" },
  btnObs: { background: "#fff8e6", color: "#a8720b", border: "1px solid #f3dfa3", borderRadius: 9, padding: "10px 12px", fontFamily: "'Manrope',-apple-system,sans-serif", fontWeight: 700, fontSize: 12.5, cursor: "pointer" },
  infoCard: { background: "#ffffff", border: "1px solid #e7ebf3", borderRadius: 18, padding: 18, display: "flex", flexDirection: "column", gap: 12 },
  infoIconBox: { width: 48, height: 48, minWidth: 48, borderRadius: 14, background: "#e5f0ff", display: "flex", alignItems: "center", justifyContent: "center" },
  infoSecTitulo: { fontWeight: 800, fontSize: 13, color: "#1a4fc0", letterSpacing: "0.04em" },
  btnVolver: { width: "100%", boxSizing: "border-box", background: "#f4f6fb", color: "#6b7488", border: "1px solid #e7ebf3", borderRadius: 12, padding: "13px 16px", fontFamily: "'Manrope',-apple-system,sans-serif", fontWeight: 700, fontSize: 13.5, cursor: "pointer" },
};
